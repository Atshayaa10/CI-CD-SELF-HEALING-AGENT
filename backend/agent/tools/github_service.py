...
# The GitHub token for the CURRENT request (set from the logged-in user's OAuth session).
# A ContextVar is async-task-safe: concurrent requests each see their own token.
# Falls back to the GITHUB_TOKEN env var for local/dev use with no OAuth configured.
_request_token: ContextVar[str] = ContextVar("gh_request_token", default="")


def set_request_token(token: str) -> None:
    """Bind the active user's GitHub token to this request's context."""
...
class GithubService:
    ...
    def _token(self):
        return _request_token.get() or os.getenv("GITHUB_TOKEN", "")
    ...
    async def file_exists(self, repo_full_name: str, file_path: str, ref: str = "main") -> bool:
        """True if a file exists in the repo at the given ref."""
    ...
    async def create_fix_branch_and_pr(
        ...
        Returns {"pr_url", "branch", "head_sha"} so the caller can CI-gate the merge.
        """
        ...
        async with httpx.AsyncClient() as client:
            ...
            html_url = pr_r.json().get("html_url", f"PR creation failed: {pr_r.text[:100]}")
            return {"pr_url": html_url, "branch": branch_name, "head_sha": new_commit_sha}

    async def wait_for_checks(self, repo_full_name: str, ref: str, timeout: int = 600, poll: int = 10) -> dict:
        """
        Polls the CI check-runs for a commit/branch until they settle or `timeout` elapses.
        This is the CI gate: the agent only merges code GitHub itself has marked green.

        Returns {"state": "success"|"failure"|"pending"|"none", "summary": str}.
        "none" means the repo has no CI configured on this ref — the caller decides whether
        the local sandbox verification is enough to proceed.
        """
        ...
        async with httpx.AsyncClient() as client:
            while True:
                try:
                    ...
                    return {"state": "pending", "summary": f"Could not read checks: {e}"}

                if not runs:
                    # No GitHub Actions checks attached to this commit.
                    return {"state": "none", "summary": "No CI checks configured on the fix branch."}
                ...
                if failed:
                    names = ", ".join(c.get("name", "check") for c in failed)
                    return {"state": "failure", "summary": f"CI failed: {names}"}

                if len(completed) == len(runs):
                    names = ", ".join(c.get("name", "check") for c in runs)
                    return {"state": "success", "summary": f"All CI checks passed: {names}"}

                if time.time() >= deadline:
                    return {"state": "pending", "summary": f"CI still running after {timeout}s; not merging."}
